...
(async () => {
  ...
  // ✅ Backup home route (prevents plain "/" from showing Not Found)
  // In production, serveStatic will handle it anyway, but this keeps things safe.
  app.get("/", (_req, res, next) => {
    if (process.env.NODE_ENV === "production") return next(); // let serveStatic handle it
    res.status(200).send("Server is running ✅ (dev mode)");
  });
  ...
  // IMPORTANT: vite only in development
